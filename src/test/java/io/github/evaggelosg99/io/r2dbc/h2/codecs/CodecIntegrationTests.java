...
import io.github.evaggelosg99.io.r2dbc.h2.H2Connection;
import io.github.evaggelosg99.io.r2dbc.h2.H2Result;
import io.github.evaggelosg99.io.r2dbc.h2.codecs.DefaultCodecs;
import io.github.evaggelosg99.io.r2dbc.h2.util.IntegrationTestSupport;
...
/**
 * Integration tests for {@link DefaultCodecs} testing all known codecs with
 * pre-defined values and {@code null} values.
 */
public class CodecIntegrationTests extends IntegrationTestSupport {
	...
	void shouldEncodePeriodAsIntervalMonth() {
		testType(connection, "INTERVAL MONTH(18)", Period.ofMonths(Integer.MAX_VALUE),
				Interval.ofMonths(Integer.MAX_VALUE));
	}
	...
	void shouldEncodeDurationAsIntervalMinute() {
		testType(connection, "INTERVAL MINUTE(18)", Duration.ofMinutes(Integer.MAX_VALUE),
				Interval.ofMinutes(Integer.MAX_VALUE));
	}
	...
	void shouldEncodeDurationAsIntervalSecond() {
		testType(connection, "INTERVAL SECOND(18,9)", Duration.ofSeconds(3, 141592654),
				Interval.ofSeconds(3, 141592654));
	}
	...
	void shouldEncodeDurationAsIntervalDayToHour() {
		testType(connection, "INTERVAL DAY(18) TO HOUR", Duration.ofDays(365).plusHours(23),
				Interval.ofDaysHours(365, 23));
	}
	...
	void shouldEncodeDurationAsIntervalDayToMinute() {
		testType(connection, "INTERVAL DAY(18) TO MINUTE", Duration.ofDays(666).plusHours(7).plusMinutes(42),
				Interval.ofDaysHoursMinutes(666, 7, 42));
	}

	@Test
	void shouldEncodeIntervalAsIntervalDayToSecond() {
		testType(connection, "INTERVAL DAY(18) TO SECOND(9)",
				Interval.ofDaysHoursMinutesNanos(666, 13, 7, 31415926535L));
	}

	@Test
	void shouldEncodeDurationAsIntervalDayToSecond() {
		testType(connection, "INTERVAL DAY(18) TO SECOND(9)",
				Duration.ofDays(666).plusHours(13).plusMinutes(7).plusSeconds(31).plusNanos(415926535),
				Interval.ofDaysHoursMinutesNanos(666, 13, 7, 31415926535L));
	}
	...
	void shouldEncodeDurationAsIntervalHourToMinute() {
		testType(connection, "INTERVAL HOUR(18) TO MINUTE", Duration.ofHours(Integer.MAX_VALUE).plusMinutes(59),
				Interval.ofHoursMinutes(Integer.MAX_VALUE, 59));
	}
	...
	void shouldEncodeDurationAsIntervalHourToSecond() {
		testType(connection, "INTERVAL HOUR(18) TO SECOND(9)",
				Duration.ofHours(13).plusMinutes(7).plusSeconds(31).plusNanos(415926535L),
				Interval.ofHoursMinutesNanos(13, 7, 31415926535L));
	}

	@Test
	void shouldEncodeIntervalAsIntervalMinuteToSecond() {
		testType(connection, "INTERVAL MINUTE(18) TO SECOND(9)",
				Interval.ofMinutesNanos(Integer.MAX_VALUE, 31415926535L));
	...
	void shouldEncodeDurationAsIntervalMinuteToSecond() {
		testType(connection, "INTERVAL MINUTE(18) TO SECOND(9)",
				Duration.ofMinutes(Integer.MAX_VALUE).plusNanos(31415926535L),
				Interval.ofMinutesNanos(Integer.MAX_VALUE, 31415926535L));
	}
	...
	void shouldEncodeZonedDateTimeAsTimestampWithTimeZone() {
		final ZonedDateTime value = ZonedDateTime.of(LocalDateTime.parse("2018-11-08T11:08:28.2"), ZoneId.of("UT"));

		testType(connection, "TIMESTAMP(1) WITH TIME ZONE", value, value.toOffsetDateTime());
	}

	@Test
	void shouldEncodeEnumAsValueEnum() {

		testType(connection, "ENUM ('A','B','C')", test.B, "B");
	}
	...
	void shouldDecodeScalarNull() {
		...
		Flux.from(connection.createStatement("INSERT INTO codec_test values($1)").bind("$1", "foo").execute())
				.flatMap(H2Result::getRowsUpdated).as(StepVerifier::create).expectNext(1L).verifyComplete();

		connection.createStatement("SELECT null, my_col FROM codec_test").execute()
				.flatMap(it -> it.map((row, rowMetadata) -> Optional.ofNullable(row.get(0)))).as(StepVerifier::create)
				.expectNext(Optional.empty()).verifyComplete();

		connection.createStatement("SELECT null, my_col FROM codec_test").execute()
				.flatMap(it -> it.map((row, rowMetadata) -> Optional.ofNullable(row.get(0, String.class))))
				.as(StepVerifier::create).expectNext(Optional.empty()).verifyComplete();
	}

	private void testType(final H2Connection connection, final String columnType, final Object value) {
...
//            actual -> assertThat(((Object[]) actual)).containsExactly(value));
	...
	private void testType(final H2Connection connection, final String columnType, final Object value,
			final Object expectedGetObjectValue) {
		testType(connection, columnType, value, value.getClass(), expectedGetObjectValue);
	}

	private void testType(final H2Connection connection, final String columnType, final Object value,
			final Class<?> valueClass, final Object expectedGetObjectValue) {
		if (value instanceof Parameter) {
			testType(connection, columnType, value, valueClass,
					actual -> assertThat(actual).isEqualTo(((Parameter) expectedGetObjectValue).getValue()));
		} else {
			testType(connection, columnType, value, valueClass,
					actual -> assertThat(actual).isEqualTo(expectedGetObjectValue));
		}
	}

	private void testType(final H2Connection connection, final String columnType, final Object value,
			final Class<?> valueClass, final Consumer<Object> nativeValueConsumer) {

		createTable(connection, columnType);

		Flux.from(connection.createStatement("INSERT INTO codec_test values($1)").bind("$1", value).execute())
				.flatMap(H2Result::getRowsUpdated).as(StepVerifier::create).expectNext(1L).verifyComplete();
		...
		connection.createStatement("SELECT my_col FROM codec_test").execute()
				.flatMap(it -> it.map((row, rowMetadata) -> row.get("my_col"))).as(StepVerifier::create)
				.consumeNextWith(nativeValueConsumer).verifyComplete();

		connection.createStatement("SELECT my_col FROM codec_test").execute()
				.flatMap(it -> it.map((row, rowMetadata) -> row.get("my_col"))).as(StepVerifier::create)
				.consumeNextWith(nativeValueConsumer).verifyComplete();

		Flux.from(connection.createStatement("UPDATE codec_test SET my_col = $1").bindNull("$1", value.getClass())
				.execute()).flatMap(H2Result::getRowsUpdated).as(StepVerifier::create).expectNext(1L).verifyComplete();

		connection.createStatement("SELECT my_col FROM codec_test").execute()
				.flatMap(
						it -> it.map((row, rowMetadata) -> Optional.ofNullable((Object) row.get("my_col", valueClass))))
				.as(StepVerifier::create).expectNext(Optional.empty()).verifyComplete();
	}

	private void createTable(final H2Connection connection, final String columnType) {

		connection.createStatement("DROP TABLE IF EXISTS codec_test").execute().flatMap(H2Result::getRowsUpdated)
				.onErrorResume(e -> Mono.empty())
				.thenMany(connection.createStatement("CREATE TABLE codec_test (my_col " + columnType + ")").execute()
