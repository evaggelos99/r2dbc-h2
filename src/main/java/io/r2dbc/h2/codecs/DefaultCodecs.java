...
public final class DefaultCodecs implements Codecs {
    ...
    public <T> T decode(final Value value, final int dataType, final Class<? extends T> type) {
        ...
        throw new IllegalArgumentException(String.format("Cannot decode value: %s of type %s", value, type.getName()));
    ...
    static List<Codec<?>> createCodecs(final Client client, final ClassLoader classLoader, final Codecs codecs) {
        ...
        return Stream.concat(
            Stream.concat(Stream.of(new BigDecimalCodec(), new BlobToByteBufferCodec(client), new BlobCodec(client), new BooleanCodec(), new ByteCodec(), new BytesCodec(), new ClobToStringCodec(client), new ClobCodec(client), new DoubleCodec(), new FloatCodec(), new IntegerCodec(), new JsonCodec(), new LocalDateCodec(), new LocalDateTimeCodec(client), new LocalTimeCodec(), new LongCodec(), new OffsetDateTimeCodec(client), new OffsetTimeCodec(client), new ShortCodec(), new StringCodec(), new UuidCodec(), new ZonedDateTimeCodec(client), new InstantCodec(client), new IntervalCodec(), new PeriodCodec(), new DurationCodec(), new SqlTimestampCodec(), new EnumCodec()), addOptionalCodecs(classLoader)), Stream.of(
                // De-prioritized codecs, must be added after optional codecs to avoid stack
                // overflow
                new ArrayCodec(codecs), new ParameterCodec(codecs))).collect(Collectors.toList());
