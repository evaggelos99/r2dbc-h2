...
/**
 * An implementation of {@link Result} representing the results of a query
 * against an H2 database.
 */
public final class H2Result implements Result {
	...
	public <T> Flux<T> map(BiFunction<Row, RowMetadata, ? extends T> f) {
		...
		return this.rows.map(row -> f.apply(row, this.rowMetadata));
	}

	@Override
	public String toString() {
		return "H2Result{" + ", rowMetadata=" + this.rowMetadata + ", rows=" + this.rows + ", rowsUpdated="
				+ this.rowsUpdated + '}';
	}
	...
	static H2Result toResult(Codecs codecs, ResultInterface result, @Nullable Long rowsUpdated) {
		...
		Flux<H2Row> rows = Flux.fromIterable(iterable).map(values -> H2Row.toRow(values, result, codecs, rowMetadata))
