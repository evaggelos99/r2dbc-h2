...
import io.github.evaggelos99.r2dbc.h2.H2DatabaseExceptionFactory.H2R2dbcNonTransientResourceException;
...
/**
 * An implementation of {@link ConnectionFactory} for creating connections to an
 * H2 database.
 */
public final class H2ConnectionFactory implements ConnectionFactory {
	...
	/**
	 * Create a new {@link CloseableConnectionFactory In-Memory database connection
	 * factory} for database at {@code name}.
	 * <p>
	 * The closeable {@link ConnectionFactory} keeps the database open unless the
	 * connection factory is {@link Closeable#close() closed}.
	 * <p>
	 * The database gets created if it does not exist yet.
	 * <p>
	 * Connecting to an existing database closes the database for all participating
	 * components if the resulting {@link ConnectionFactory} is
	 * {@link CloseableConnectionFactory#close() closed}.
	 *
	...
	/**
	 * Create a new {@link CloseableConnectionFactory In-Memory database connection
	 * factory} for database at {@code name} given {@code username} and
	 * {@code password}.
	 * <p>
	 * The closeable {@link ConnectionFactory} keeps the database open unless the
	 * connection factory is {@link Closeable#close() closed}.
	...
	public static CloseableConnectionFactory inMemory(String name, String username, CharSequence password,
		...
		H2ConnectionConfiguration.Builder builder = H2ConnectionConfiguration.builder().inMemory(name)
	...
	public Mono<H2Connection> create() {
		return this.clientFactory.map(client -> new H2Connection(client, new DefaultCodecs(client)));
	}
		...
		public Mono<H2Connection> create() {
			return Mono.fromSupplier(() -> {
				...
				if (this.persistentConnection == null) {
					throw new H2R2dbcNonTransientResourceException(
							String.format("ConnectionFactory for %s is closed", this.configuration.getUrl()));
				}
